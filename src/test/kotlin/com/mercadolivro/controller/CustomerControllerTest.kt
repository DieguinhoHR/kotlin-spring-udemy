...
class CustomerControllerTest {
    ...
    fun `should filter all customers by name when get all`() {
        ...
        val andExpect = mockMvc.perform(get("/customers?name=Gus"))//invoca endpoint
            .andExpect(status().isOk)//valida status
            .andExpect(jsonPath("$.length()").value(1)) //valida o tamanho da lista
            .andExpect(jsonPath("$[0].id").value(customer1.id)) //valida path
            .andExpect(jsonPath("$[0].name").value(customer1.name))
            .andExpect(jsonPath("$[0].email").value(customer1.email))
            .andExpect(jsonPath("$[0].status").value(customer1.status.name))
    ...
    @Test
    fun `should return error with create customer with invalid information`() {
    ...
    fun `should return get user id when user is admin`() {
        val customer = customerRepository.save(buildCustomer())
